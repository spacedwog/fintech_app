...
# ── 2. Baixar Tailwind CSS em style/ (se não existir) ─────────────────────
...
if not os.path.exists(tw_path):
    ...
    print("style/tailwind.min.css já existe, pulando download.")

# ── 3. Criar PDF com link do repositório ──────────────────────────────────
try:
    ...
    story = [
        ...
        Paragraph("Repositório público no GitHub:", label_style),
        Paragraph("https://github.com/spacedwog/fintech_app", link_style),

        HRFlowable(width="100%", thickness=1, color=colors.HexColor("#e5e7eb")),
        Spacer(1, 0.4*cm),

        Paragraph("Descrição do projeto:", label_style),
        Paragraph(
            "Tela de Dashboard do sistema Fintech Inteligente, desenvolvida com HTML semântico, "
            "CSS customizado (style.css) e Tailwind CSS. A interface apresenta KPIs financeiros, "
            "gráfico de barras de gastos mensais, distribuição por categoria e lista de despesas "
            "recentes. O layout é totalmente responsivo para dispositivos mobile.",
            body_style,
        ),

        Spacer(1, 0.4*cm),
        Paragraph("Arquivos incluídos no ZIP:", label_style),
